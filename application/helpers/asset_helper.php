...
function image_att($asset_name, $attributes)
{
	if ($attributes != '')
	{
		$attributes = _parse_asset_html($attributes);
	}
	return '<img src="'.image_url($asset_name).'"'.$attributes.' />';
}

// Asset javaScript location
function js_url($asset_name, $module_name = NULL, $user = NULL)
{
	if($user == NULL) {
...
function js($asset_name, $module_name = NULL, $user = NULL)
{
	if($user == NULL) {
		return '<script type="text/javascript" src="'.js_url($asset_name, $module_name).'"></script>';
	} else {
		return '<script type="text/javascript" src="'.js_url($asset_name, $module_name, $user).'"></script>';
  }
}
